...
ctrl.addBinding(config, 'exploded', {
  label: 'explode',
}).on('change', () => {
...
const recordKey = (configItem) => (event) => {
  configItem.key = event.key;
  ctrl.refresh();
};
...
for (const id of ids) {
  ...
  keyFolder.addBinding(config[id], 'text', {
    label: 'text',
  }).on('change', () => {
  ...
  keyFolder.addBinding(config[id], 'travel', {
    min: 1,
    max: 50,
    step: 1,
  }).on('change', () => {
  ...
  keyFolder.addBinding(config[id], 'hue', {
    min: 0,
    max: 360,
    step: 1,
  }).on('change', () => {
  ...
  keyFolder.addBinding(config[id], 'saturation', {
    min: 0,
    max: 2,
    step: 0.1,
  }).on('change', () => {
  ...
  keyFolder.addBinding(config[id], 'brightness', {
    min: 0,
    max: 2,
    step: 0.1,
  }).on('change', () => {
...
if (keypad) keypad.style.setProperty('opacity', 1);
